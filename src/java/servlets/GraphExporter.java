...
import cytoscape.visual.ui.editors.discrete.CyComboBoxPropertyEditor.Value;
...
public class GraphExporter extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (isImage) { //use Image
			...
			Image test = getNetworkAsImage(request, response);
			ServletOutputStream output = response.getOutputStream();
			ImageIO.write((RenderedImage) test, "png", output);
	...
	private Image getNetworkAsImage(HttpServletRequest request,
		...
		Image image = ImageIO.read(request.getInputStream());
		return image;
	}
