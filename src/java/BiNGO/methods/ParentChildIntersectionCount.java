...
public class ParentChildIntersectionCount implements DistributionCount {
    ...
    public ParentChildIntersectionCount(Annotation annotation, Ontology ontology, HashSet selectedNodes,
        ...
        ParentChildIntersectionCount.annotation = annotation;
        ParentChildIntersectionCount.ontology = ontology;
        ParentChildIntersectionCount.alias = alias;
        annotation.setOntology(ontology);

        ParentChildIntersectionCount.selectedNodes = selectedNodes;
        ParentChildIntersectionCount.refNodes = refNodes;
    }
...
*/
	...
	public void countBigN() {
        ...
        mapBigN = new HashMap();
        for (Object id : ParentChildIntersectionCount.mapSmallX.keySet()) {
            int[] parents = ParentChildIntersectionCount.ontology.getTerm(((Integer) id).intValue()).getParentsAndContainers();
            int bigN = 0;
            for (Object i : ParentChildIntersectionCount.refNodes) {
	...
	public void countBigX() {
        ...
        for (Object id : ParentChildIntersectionCount.mapSmallX.keySet()) {
            int[] parents = ParentChildIntersectionCount.ontology.getTerm(((Integer) id).intValue()).getParentsAndContainers();
            int bigX = 0;
            for (Object i : ParentChildIntersectionCount.selectedNodes) {
                HashSet classifications = getNodeClassifications(i.toString());
                boolean ok = true;
                for (int j : parents) {
                    if (!classifications.contains(j + "")) {
